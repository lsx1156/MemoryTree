...
| 测试项 | 说明 | 覆盖场景 |
|-------|------|---------|
| `generate_ollamaUnavailable_throwsRuntimeExceptionWithFriendlyMessage` | 模拟 Ollama 服务不可用（错误端口），验证抛出含友好消息的 RuntimeException | 网络鲁棒性：模型服务不可用 |
| `buildRequestJson_superLongInput_doesNotCrash` | 5000 字符超长输入测试，验证 JSON 构建不崩溃 | 推理鲁棒性：异常输入处理 |
| `kvCache_concurrentAccess_noDeadlock` | 10 线程 × 20 轮并发 KV cache 操作，验证无死锁无数据竞争 | 并行鲁棒性：高并发 + 资源竞争 |

**测试总数：** 55 → 58（+3）
**鲁棒性覆盖：** 14.1 表格中 ❌ → ✅ 共 6 项（异常输入、索引一致性、高并发、资源竞争、参数边界、模型服务不可用），覆盖 8/12
**README 鲁棒性章节：** 实事求是更新实现状态标记，附测试依据和 ⚠️ 项说明
...
| **README 鲁棒性测试标记修正** | 第十四章鲁棒性测试表格原标记为 ✅，与"无自动化测试代码"声明自相矛盾；现统一改为 ❌ 以反映未经测试验证的真实状态 | [README.md](file:///e:/AI/MemoryTree/README.md) |
...
> **说明**：以下为**设计标准与验收规范**。当前已有 58 项自动化测试覆盖核心路径（单元测试 + 集成测试）。✅ 表示已通过自动化测试验证，⚠️ 表示部分覆盖（核心路径已测但未完全达标），❌ 表示尚未覆盖。
...
**已覆盖项的测试依据（8/12）：**
- 异常输入处理：`buildRequestJson_superLongInput_doesNotCrash`（5000 字符超长输入）+ `calculateConfidence_returnsBaselineForEmptyText`（空/null 输入）
- 索引一致性：`delete_removesEntry`（删除后不可检索）+ `query_byKeyword_returnsMatchingEntries`（索引查询正确）
- 高并发测试：`kvCache_concurrentAccess_noDeadlock`（10 线程 × 20 轮并发无死锁）
- 资源竞争：`kvCache_concurrentAccess_noDeadlock`（同上，验证无数据竞争）
- 参数边界测试：`calculateConfidence_neverExceedsUpperBound`（≤0.95）+ `calculateReward_neverBelowHalf`（≥0.5）
- 模型服务不可用：`generate_ollamaUnavailable_throwsRuntimeExceptionWithFriendlyMessage`（错误端口模拟 Ollama 不可用）
...
**⚠️ 项说明：**
- 推理失败：Ollama 不可用时抛出含友好消息的 RuntimeException（已测），但无自动重试机制
- 记忆存储失败：`persist()` 失败时记录日志并抛出 RuntimeException（已测），但无回滚到上一状态
...
**性能指标说明：**
- 推理响应时间：集成测试实测 3717ms（1+1 推理，139 tokens），远低于 30 秒要求
- 内存占用：已改为 `Runtime.getRuntime()` 实时 JVM 堆内存（非硬编码），但未添加 < 8GB 断言测试
